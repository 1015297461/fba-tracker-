...
export function TweakRadio({ label, value, options, onChange }: {
  ...
  const segLookup: Record<number, number> = { 2: 16, 3: 10 };
  const fitsAsSegments = maxLen <= (segLookup[options.length] ?? 0);
  if (!fitsAsSegments) {
        ...
        {options.map((o: any, i: number) => {
          ...
          return (
            <button key={i} type="button" className="twk-chip" role="radio"
                    ...
                    onClick={() => onChange(o)}>
              {sup.length > 0 && (
                <span>{sup.map((c: string, j: number) => <i key={j} style={{ background: c }} />)}</span>
              )}
              {on && <__TwkCheck light={isLight(hero)} />}
            </button>
          );
        })}
      </div>
