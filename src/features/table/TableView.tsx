...
import { calcProfit, calcVariantProfit } from '../../data/calc';
import { useProducts } from '../../context/ProductContext';
import type { Product } from '../../data/types';

function lastExpectedShip(p: Product): string | null {
...
function calcOrderTotal(p: Product): number {
...
function getColVal(p: Product, k: string): any {
  ...
  switch(k) {
    ...
    case 'bomTotal': return (p.stages.bom?.items||[]).reduce((s: number, i: any) => s+(Number(i.qty)||0)*(Number(i.unitCost)||0), 0) || null;
...
export function TableView({ onSelectProduct, filter }: { onSelectProduct: (id: string) => void; filter: string }) {
  const { products: allProducts } = useProducts();
  const products = filter && filter !== 'all'
    ? allProducts.filter((p: Product) => p.status === filter)
    : allProducts;
  ...
  function exportExcel() {
    const doExport = () => {
      ...
      const headers = visibleCols.map((c: any) => c.label.replace(' 🔒', ''));
      const data = rows.map((p: Product) => {
        ...
        return visibleCols.map((c: any) => {
          switch(c.k) {
            ...
            case 'bomTotal': return +(p.stages.bom?.items||[]).reduce((s: number, i: any)=>s+(Number(i.qty)||0)*(Number(i.unitCost)||0),0).toFixed(2) || '';
      ...
      ws['!cols'] = visibleCols.map((c: any) => ({ wch: c.k === 'name' ? 28 : c.num ? 12 : 16 }));
  ...
  function renderCell(p: Product, k: string, m: any, pr: any, hasV: boolean, isExp: boolean) {
    switch(k) {
      case 'name': return (
        <td key="name" className="sticky pname">
          {hasV && (
            <button className="variant-expand-btn" title={isExp ? '收起变体' : '展开变体'}
              onClick={(e: React.MouseEvent) => { e.stopPropagation(); toggleExpand(p.id); }}>
              {isExp ? '▾' : '▸'}
            </button>
          )}
      ...
      case 'compPrice': return <td key="compPrice" className="num">{p.stages.research?.avgPrice?`$${Number(p.stages.research.avgPrice).toFixed(2)}`:'—'}</td>;
      ...
      case 'bomTotal': {
        const t = (p.stages.bom?.items||[]).reduce((s: number, i: any) => s+(Number(i.qty)||0)*(Number(i.unitCost)||0), 0);
  ...
  function renderVariantCell(v: any, k: string, vi: number, p: Product, vm: any, vpr: any, vBomTotal: number) {
    switch(k) {
      case 'name': return (
      ...
      <div className="dtable-wrap">
        <div className="dtable-toolbar">
          ...
          <span style={{marginLeft:'auto', display:'flex', gap:6, position:'relative'}}>
            ...
            {showColPanel && (
              <div className="col-panel" onClick={(e: React.MouseEvent) => e.stopPropagation()}>
                      ...
                      {gCols.map(c => (
                        <label key={c.k} className="col-panel-item">
                          <input type="checkbox"
                            ...
                            disabled={!!(c as any).alwaysVisible}
                            onChange={() => { if (!(c as any).alwaysVisible) toggleCol(c.k); }} />
        ...
        <div className="dtable-scroll">
          <table className="dtable">
            <thead>
              ...
              <tr>
                {visibleCols.map(c => (
                  <SortTh key={c.k} k={c.k} label={c.label} num={c.num} sticky={(c as any).sticky} />
                ))}
              </tr>
            </thead>
            <tbody>
              {rows.map((p: Product) => {
                ...
                return (
                  <React.Fragment key={p.id}>
                    ...
                    {hasV && isExp && (p.variants||[]).map((v: any, vi: number) => {
                      const vm = calcVariantProfit(v, p.fxRate);
                      const vpr = v.stages?.profit || {};
                      const vBomTotal = (v.stages?.bom?.items||[]).reduce((s: number, i: any) => s+(Number(i.qty)||0)*(Number(i.unitCost)||0), 0);
                      return (
